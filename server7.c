...
else if(strcmp(nume,"/links")==0) strcpy(fis,"link.txt");
...
if(strcmp(functie,"/insert")==0)
	...
	bzero(melodie_inserata,500);
        file=open("link.txt",O_APPEND|O_WRONLY);
        strcat(melodie_inserata,nume_melodie);
        strcat(melodie_inserata,"-");
...
char ch,mesaj[1000],functie[10],nume_melodie[30],descriere[200],nume_aux[30],au[30],aux[100],link[100];

separare_mk(comanda,functie,nume_melodie,descriere);
if(strcmp(functie,"/mkcom")==0)
{
        file1=open("link.txt",O_RDONLY);
